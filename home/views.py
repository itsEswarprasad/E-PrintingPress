...
def PageObjects(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if(name in ['idupulapaya','vempalli','kadiri','duvvur']):
                new_author.name_of_pub="eswar"
            elif(name in ['marripaadu','rajampet','rajupalem','kamalapuram']):
                 new_author.name_of_pub="reshma"
            else:
                new_author.name_of_pub="gayathri"
            ...
            new_author.payment=price-price*0.3
            ...
            if(name_of_pub=="eswar"):
                to_add="r170243@rguktrkv.ac.in"
            elif(name_of_pub=="reshma"):
                to_add="r170205@rguktrkv.ac.in"
            else:
                to_add="r170204@rguktrkv.ac.in"
...
def history(request):
    ...
    co=op.objects.filter(user_name=u_name).count()
   # u_name=request.user.id
    return render(request,"user/history.html",{"dict":data,"cou":co})
def forgot(request):
